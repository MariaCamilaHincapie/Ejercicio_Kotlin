...
fun getCities():List<String> {
    val cities = customers.map { it.city }.toHashSet().toString()
...
fun getCustomerByName(name: String): Customer? {
    return customers.find { it.name.equals(name)}
}
...
fun getCustomerWithMostMoney(): Customer? {
    val costWithMostMoney =  customers.maxByOrNull { it.calculateBalance() }
    return costWithMostMoney
}
...
fun getCustomerWithLeastMoney(): Customer? {
    val costWithLeastMoney = customers.minByOrNull { it.calculateBalance() }
    return costWithLeastMoney
}
...
fun sortCustomerByBalanceDesc(): List<Customer> {
    val sortCustomers = customers.sortedBy { it.calculateBalance() }
    return listOf()
}